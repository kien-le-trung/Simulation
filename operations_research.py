...
spec = importlib.util.spec_from_file_location("patient_simulation", PATIENT_SIM_PATH)
patient_simulation = importlib.util.module_from_spec(spec)
spec.loader.exec_module(patient_simulation)
PatientModel = patient_simulation.PatientModel
...
def run_controller(n_trials=250, seed=7):
    ...
    patient = PatientModel(seed=seed)  # your simulator
...
def visualize(hist, counts_5x5):
    d = np.array(hist["d"])
    t = np.array(hist["t"])
    hit = np.array(hist["hit"])
    p_pred = np.array(hist["p_pred"])
    v_req = np.array(hist["v_req"])
    v_hat = np.array(hist["v_hat"])
    sigma_v = np.array(hist["sigma_v"])

    # 1) trajectory of (d,t)
    plt.figure()
    plt.scatter(d, t, s=10)
    plt.title("Chosen (d,t) points over trials")
    plt.xlabel("Distance d (m)")
    plt.ylabel("TTL t (s)")
    plt.show()

    # 2) 5x5 variability heatmap
    plt.figure()
    plt.imshow(counts_5x5, aspect="auto")
    plt.title("Selection counts across 5x5 bins (distance x time)")
    plt.xticks([0,1,2,3,4], ["shortest","short","medium","long","longest"])
    plt.yticks([0,1,2,3,4], ["closest","close","medium","far","farthest"])
    for i in range(5):
        for j in range(5):
            plt.text(j, i, str(counts_5x5[i,j]), ha="center", va="center", fontsize=8)
    plt.colorbar()
    plt.show()

    # 4) predicted p_hit vs time
    plt.figure()
    plt.plot(p_pred)
    plt.plot(rolling_mean(hit, w=10), label="Rolling hit rate", linestyle="--")
    plt.ylim(-0.05, 1.05)
    plt.title("Predicted vs. actual average (every 5 trials) hit probability for chosen action")
    plt.xlabel("Trial")
    plt.ylabel("p_pred")
    plt.legend()
    plt.show()

    # 5) required velocity vs learned velocity
    plt.figure()
    plt.plot(v_req, label="v_req = d/t")
    plt.plot(v_hat, label="v_hat (learned)")
    plt.plot(v_hat + 2*sigma_v, linestyle="--", label="v_hat + 2σ")
    plt.plot(v_hat - 2*sigma_v, linestyle="--", label="v_hat - 2σ")
    plt.title("Velocity: required vs learned (±2σ)")
    plt.xlabel("Trial")
    plt.ylabel("m/s")
    plt.legend()
    plt.show()
