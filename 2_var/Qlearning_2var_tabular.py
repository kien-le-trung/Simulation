...
# ----------------------------
# Reward (same design as 1var-tabular)
# ----------------------------
def compute_reward(hit: bool, dist_ratio: float, time_ratio: float) -> float:
    ...
    HIT:  r = time_ratio in [0, 1]
        ...
        toward challenging but achievable (d, t) pairs.
...
class TabularQLearner:
    """
    Tabular Q-learner over a 2D (d, t) lattice.

    State  : (d_idx, t_idx) — position on the n_d × n_t grid
    Actions: 4 incremental moves — D_UP, D_DOWN, T_UP, T_DOWN
             Moves that would leave the grid are clipped to the boundary,
             so the agent stays at the edge if it tries to move out.
    Q-table: numpy array of shape (n_d, n_t, N_ACTIONS)

    Because actions are local (±1 step), the agent must traverse the grid
    gradually rather than jumping arbitrarily — this produces smoother
    difficulty trajectories than the 1var-tabular free-jump design.
    """
    ...
    def __init__(self, cfg: QLearningConfig, rng: np.random.Generator):
        ...
        self.q_table = np.zeros((cfg.n_d, cfg.n_t, N_ACTIONS), dtype=float)
        self.epsilon = cfg.epsilon_start

    def next_state(self, d_idx: int, t_idx: int, action: int) -> Tuple[int, int]:
        """Apply action and return the resulting (d_idx, t_idx), clipped to grid."""
        if action == D_UP:
            return min(d_idx + 1, self.cfg.n_d - 1), t_idx
        if action == D_DOWN:
            return max(d_idx - 1, 0), t_idx
        if action == T_UP:
            return d_idx, min(t_idx + 1, self.cfg.n_t - 1)
        # T_DOWN
        return d_idx, max(t_idx - 1, 0)

    def select_action(self, d_idx: int, t_idx: int) -> int:
        """Epsilon-greedy over the 4 local actions."""
        if float(self.rng.uniform()) < self.epsilon:
            return int(self.rng.integers(0, N_ACTIONS))
        return int(np.argmax(self.q_table[d_idx, t_idx]))
    ...
    def update(
        ...
        action: int,
        reward: float,
        nd_idx: int, nt_idx: int,
    ) -> None:
        """Standard Q-learning (Bellman) update."""
        td_target = reward + self.cfg.gamma * float(np.max(self.q_table[nd_idx, nt_idx]))
        td_error = td_target - self.q_table[d_idx, t_idx, action]
        self.q_table[d_idx, t_idx, action] += self.cfg.alpha * td_error
...
def run_sim(
    ...
    for k in range(n_trials):
        action = agent.select_action(d_idx, t_idx)
        nd_idx, nt_idx = agent.next_state(d_idx, t_idx, action)
        ...
        agent.update(d_idx, t_idx, action, reward, nd_idx, nt_idx)
        agent.decay_epsilon()
        ...
        logs["q_max"].append(float(np.max(agent.q_table[nd_idx, nt_idx])))

        d_idx, t_idx = nd_idx, nt_idx
