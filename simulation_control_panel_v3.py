...
#
# Design philosophy:
#   Speed (k_d0_per_sec, k_d_decay) and ROM (max_reach_map) are DECOUPLED.
#   - k_d_decay models mild speed reduction with distance (Fitts'-law-like),
#     NOT range-of-motion limits.
#   - max_reach_map is the hard geometric/structural ROM ceiling per direction.
#   - spatial_strength_map modulates velocity per direction (neuromuscular).
#
# 3x3 spatial maps are flattened by idx = elevation * 3 + azimuth
# elevation: low(0) -> high(2),  azimuth: left(0) -> right(2)

PATIENT_PROFILES = {
    # ── Severity spectrum (uniform spatial) ──────────────────────────
    "overall_weak": {
        "k_d0_per_sec": 0.18,       # slow base speed
        "k_d_decay": 1.0,           # moderate Fitts-like decay
        "v_sigma0": 0.06,           # noisy
        "v_sigma_growth": 0.04,     # fatigues noticeably
        "spatial_strength_map": [1.0] * 9,
        "max_reach_map": [0.2] * 9,  # limited ROM (~45 cm)
    },
    "overall_medium": {
        "k_d0_per_sec": 0.30,
        "k_d_decay": 0.5,
        ...
        "spatial_strength_map": [1.0] * 9,
        "max_reach_map": [0.70] * 9,
    },
    "overall_strong": {
        "k_d0_per_sec": 0.65,
        "k_d_decay": 0.1,           # nearly flat speed across range
        "v_sigma0": 0.03,
        "v_sigma_growth": 0.008,
        "spatial_strength_map": [1.0] * 9,
        "max_reach_map": [1.0] * 9,   # full ROM
    },

    # ── Speed / ROM dissociation ─────────────────────────────────────
    "highspeed_lowrom": {
        # e.g. joint contracture with preserved motor recruitment
        # fast & precise within a small reachable volume
        "k_d0_per_sec": 0.65,       # genuinely fast
        "k_d_decay": 0.1,           # mild decay — stays fast within range
        "v_sigma0": 0.03,
        "v_sigma_growth": 0.01,
        "spatial_strength_map": [1.0] * 9,
        "max_reach_map": [0.2] * 9,  # hard ROM ceiling at 35 cm
    },

    "lowspeed_highrom": {
        # e.g. flaccid paresis — full passive ROM, weak/slow recruitment
        "k_d0_per_sec": 0.15,       # genuinely slow everywhere
        "k_d_decay": 0.1,           # almost flat — slow is slow
        "v_sigma0": 0.06,
        "v_sigma_growth": 0.04,
        "spatial_strength_map": [1.0] * 9,
        "max_reach_map": [1.0] * 9,  # nearly full ROM
    },
}
