...
def cartesian_params(b0_grid, b1_grid, b2_grid, b3_grid, b4_grid):
    """Return parameter lattice arrays (B0,B1,B2,B3,B4) broadcastable to likelihood computations."""
    B0 = b0_grid[:, None, None, None, None]
    B1 = b1_grid[None, :, None, None, None]
    B2 = b2_grid[None, None, :, None, None]
    B3 = b3_grid[None, None, None, :, None]
    B4 = b4_grid[None, None, None, None, :]
    return B0, B1, B2, B3, B4


def centered_dir(direction):
    """Centered direction encoding: (dir - 2) / 2 -> [-1, -0.5, 0, 0.5, 1]."""
    return (float(direction) - 2.0) / 2.0


def p_hit_model(d, t, direction, B0, B1, B2, B3, B4):
    """
    Psychometric model with speed interaction:
        v_req = d / max(t, 0.01)
        p_hit = sigmoid(B0 + B1*d + B2*t + B3*v_req + B4*dir_centered)
    """
    v_req = d / max(t, 0.01)
    dir_c = centered_dir(direction)
    return sigmoid(B0 + B1 * d + B2 * t + B3 * v_req + B4 * dir_c)


def expected_entropy_for_x(d, t, direction, posterior, B0, B1, B2, B3, B4, eps=1e-12):
    """
    ...
    p_hit = p_hit_model(d, t, direction, B0, B1, B2, B3, B4)
    p_miss = 1.0 - p_hit
...
def build_calibration_prior(calibration_result, posterior_shape,
    ...
    Weights parameter grid points by how well they predict the observed calibration hit rate.
    """
    if not calibration_result:
    ...
    # Compute calibration hit rate and typical stimulus
    n_hits = sum(1 for tr in trials if tr.get("hit", tr.get("reached", False)))
    cal_hit_rate = n_hits / max(len(trials), 1)

    # Use mid-range stimulus as reference point
    d_mid = (dmin + dmax) / 2.0
    t_mid = (tmin + tmax) / 2.0
    dir_mid = 2  # center direction

    # For each parameter grid point, compute p_hit at the reference stimulus
    p_pred = p_hit_model(d_mid, t_mid, dir_mid, B0, B1, B2, B3, B4)

    # Weight: higher for grid points that predict close to observed hit rate
    # Gaussian kernel centered on cal_hit_rate
    sigma = 0.20
...
def run_quest_plus_dt(
    ...
    p_target=0.70,
    p_tol=0.08,
    w_dir_util=0.0,  # pure entropy minimization (no direction bias)
    seed=7,
    # stimulus grid (d,t)
    d_grid=None,
    t_grid=None,
    dir_grid=None,
    # parameter grids (B0,B1,B2,B3,B4)
    b0_grid=None,
    b1_grid=None,
    b2_grid=None,
    b3_grid=None,
    ...
    """
    QUEST+ loop:
      - maintain posterior over (B0,B1,B2,B3,B4)
      - psychometric model: p_hit = sigmoid(B0 + B1*d + B2*t + B3*v_req + B4*dir_centered)
      - choose (d,t,dir) minimizing expected entropy, with adaptive exploration
    ...
    # Constrained parameter grids (plausible ranges only)
    if b0_grid is None:
        b0_grid = np.round(np.linspace(-4.0, 4.0, 7), 4)       # intercept
    if b1_grid is None:
        b1_grid = np.round(np.linspace(-8.0, -0.5, 7), 4)      # distance: always negative
    if b2_grid is None:
        b2_grid = np.round(np.linspace(0.5, 6.0, 6), 4)        # time: always positive
    if b3_grid is None:
        b3_grid = np.round(np.linspace(-6.0, -0.5, 6), 4)      # speed: always negative
    if b4_grid is None:
        b4_grid = np.round(np.linspace(-3.0, 3.0, 5), 4)       # direction: either sign

    # parameter lattice (5D)
    B0, B1, B2, B3, B4 = cartesian_params(b0_grid, b1_grid, b2_grid, b3_grid, b4_grid)
    posterior_shape = (len(b0_grid), len(b1_grid), len(b2_grid), len(b3_grid), len(b4_grid))

    # Calibration-informed prior
    posterior = build_calibration_prior(
        calibration_result, posterior_shape,
        B0, B1, B2, B3, B4,
        dmin, dmax, tmin, tmax,
    )

    # Maximum possible entropy (for adaptive exploration)
    ...
    for k in range(n_trials):
        ...
        for (d, t, direction) in stimuli:
            EH, p_pred = expected_entropy_for_x(d, t, direction, posterior, B0, B1, B2, B3, B4)
            cand.append((EH, p_pred, d, t, direction))

        # Entropy-based adaptive exploration
        H_current = entropy(posterior)
        H_ratio = H_current / H_max  # 1.0 = know nothing, ~0 = converged

        if H_ratio > 0.8:
            # High uncertainty — pure information gain, no p_target filter
            near = cand
        elif H_ratio > 0.4:
            # Medium — wide tolerance
            near = [x for x in cand if abs(x[1] - p_target) <= p_tol * 2]
        else:
            # Low uncertainty — tight p_target targeting
            near = [x for x in cand if abs(x[1] - p_target) <= p_tol]

        # Pure QUEST+ selection: minimum expected entropy
        chosen = min(near, key=lambda c: c[0]) if len(near) > 0 else min(cand, key=lambda c: c[0])
        ...
        p_hit = p_hit_model(d_sys, t_sys, direction, B0, B1, B2, B3, B4)
